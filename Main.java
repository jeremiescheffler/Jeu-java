	...
	public static void main(String[] args) {
		...
		try{
			...
			for(int i=0; i<nombrePlayer; i++){
				System.out.println("Ajouter une IA sur un joueurs(N pour quitter)");
				int x = sc.nextInt();
				listeIA[x-1] = x;
		...
		//on modifie les parametres des joueurs
		ajoutPlayer();
		
		
		//BOUCLE DU JEU : 
		//boucle tant que la partie n'est pas terminee
		...
		while(partieTerminee == false){
			...
			//on incremente la variable courantPlayer
			joueursSuivant();
			
			//tant que la couleur n'est pas valable on redemande
			//si l'utilisateur ne rentre aucune lettre on demande si il veut quitter
			try{
				...
				System.out.println("(Tapez sur entree a nouveau pour quitter)");
			...
			switch (courantPlayer){
			 	case 1:
			 		...
			 		//on verifie si le joueur 1 a gagne
...
//=============================================================
	...
	public static void quiEstGagnant(){
		...
		//on vérifie le cas d'egalité 
		for (int k=0; k<4; k++){
			if (points[k]==max && k!=indice_max){
				egalite=true; 
				System.out.println("Il y a égalité entre le joueur"+(indice_max+1)+" et "+(k+1)+" avec "+(max)+"points");
				points[k]=-1;
			}
		}
		points[indice_max]=-1; 
		...
		if (egalite==false){
			System.out.println("Le joueur " +(indice_max+1)+ " remporte la partie avec un total de "+max+" points!!");
		}
	...
	public static void ajoutPlayer(){
		//on ajoute des joueurs et on definit leurs parametres 
	...
	public static boolean choixCouleur(char coul, int player){
		 ...
		 * n'est pas utilisee par les adversaires
		 */
		char[] all = {'R','V','B','I','J','O'};
